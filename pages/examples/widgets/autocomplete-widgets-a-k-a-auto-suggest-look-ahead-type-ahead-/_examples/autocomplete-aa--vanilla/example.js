...
function autocomplete (inputEl, listFromValue, prefs) {
  ...
  function handleDocumentFocus (event) {
    ...
    if (
      !containsElement(inputEl, event.target) &&
      !containsElement(suggestionsEl, event.target)
    ) {
      close()
      stop()
    }
  ...
  function selectSuggestion (value) {
    ...
    selectAll()
    say(
      fillString(options.i18n.selected, {
        selected: selected + 1,
  ...
  function complete () {
    ...
    completed = true
    say(
      fillString(options.i18n.complete, {
        value: inputEl.value
      })
    )
    inputEl.focus()
  ...
  function updateSuggestions () {
    ...
    listFromValue(val, function (list) {
      ...
      if (list.length) {
        ...
        close()
        say(
          fillString(options.i18n.notfound, {
            count: 0,
            value: val
          }),
          1500
