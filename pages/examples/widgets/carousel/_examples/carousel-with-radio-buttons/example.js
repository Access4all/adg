class AdgCarousel {
  constructor (el) {
    this.element = el
    this.directionButtons = this.element.querySelectorAll('button[data-carousel-direction]')
    this.autoplayToggles = this.element.querySelectorAll('button[data-carousel-autoplay]')
    this.init()
  }

  init () {
    this.directionButtons.forEach((button, buttonIndex, buttonArray) => {
      console.log(button)
    })
    this.autoplayToggles.forEach((toggle, toggleIndex, toggleArray) => {
      console.log(toggle)
    })
  }
}
