class AdgCarousel {
  constructor (el) {
    this.element = el
    this.directionControls = this.element.querySelectorAll('button[data-carousel-direction]')
    this.jumpControls = this.element.querySelectorAll('input[type=radio]')
    this.autoplayControl = this.element.querySelector('button[data-carousel-autoplay]')
  ...
  init () {
    ...
    this.directionControls.forEach((button) => {
      button.addEventListener('click', (event) => {
        ...
        const allPanels = this.element.querySelectorAll(`input[name=${carouselId}]`)
        const currentIndex = Math.max([...allPanels].findIndex(jumpControl => jumpControl.checked === true), 0)
        ...
        this.pushAlertMessage(`Showing panel ${nextIndex + 1} of ${maxIndex + 1}`)
      })
    })
    ...
    setInterval(() => {
      if (this.autoplayControl.getAttribute('aria-pressed') === 'true') {
        this.element.querySelector('[data-carousel-direction=next]').dispatchEvent(clickEvent)
      }
    }, 3000)
