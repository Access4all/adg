...
- In Firefox this works like a charm (both JAWS and NVDA).
    - In the example, the radio buttons for place are correctly announced, like "Place of Residence grouping. Urban Area. An area with a dense concentration of buildings ...".
- In Internet Explorer things are messed up when elements are packed into a `<fieldset>`/`<legend>` structure.
    - In the example, the radio buttons for place are both announced like "Place of Residence. Name. Please tell us your full name."
...
And for associating additional content to a control ("Please tell us ..."), we advise to stick to better solutions (if you are really curious and want to learn more about this, skip ahead and read [Placing non-interactive content between form controls](/examples/forms/non-interactive-content)), at least if you are using `<fieldset>`/`<legend>` structures.
