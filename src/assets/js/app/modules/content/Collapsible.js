...
export default class Collapsible extends BaseModule {
	...
	init(element) {
		...
		this.$el = $(element);
		this.options = $.extend({}, this.DEFAULT_OPTIONS, this.$el.data('collapsible-options'));
	...
	validate() {
		if (this.getToggleElement().is(this.INPUT) && this.getToggleElement().prop('checked')) {
	...
	onToggleClick(e) {
		if (!this.getToggleElement().hasClass('clickable')) {
			if (!this.getToggleElement().is(this.INPUT) || (this.getToggleElement().is(this.INPUT) && !this.getToggleElement().prop('checked'))) {
				e.preventDefault();
			}
		}

		if (this.getToggleElement().hasClass('is-opened') &&
	...
	getToggleElement() {
		let $toggles = this.$el.find('.js-Collapsible--toggle, .js-Collapsible--toggle-dummy');
	...
	getContainerElement() {
		return this.$container || (this.$container = (typeof this.options.containerSelector !== 'undefined') ? $(this.options.containerSelector) : this.$el.find('.js-Collapsible--container').first());
	}
};
